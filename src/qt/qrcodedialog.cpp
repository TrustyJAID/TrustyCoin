...
{
    QString ret = QString("trustycoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non TYC unit input in lnReqAmount, we generate the URI with TYC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(TrustycoinUnits::format(TrustycoinUnits::TYC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
